...
_BreakUpResult = namedtuple('BreakUpResult', ['lastname', 'givenname', 'initials', 'prefix'])
...
def _breakUp(name):
    ...
    for expression in compiledSpecificExpressions:
        ...
        if m:
            if 'result' in m.groupdict():
                return _BreakUpResult(m.groupdict()['result'].lower(), '', '', '')
            else:
                return None
    for expression, initialsPostProcess in compiledExpressions:
        ...
        if m:
            ...
            givenname = m.groupdict().get('initials')
            givenname = '' if givenname is None else givenname.lower().strip()
            if initialsPostProcess:
                initials = addDots(initialsPostProcess(givenname))
            else:
                initials = ''
            prefix = m.groupdict().get('prefix', "")
            return _BreakUpResult(str(lastname), str(givenname), str(initials), str(prefix))
    return None
...
def _helsing_v(parts):
    prefix = ''.join([aChar == "'t" and aChar or aChar+"." for aChar in getInitials(parts.prefix)])
    result = _capitalize(parts.lastname)
    if prefix:
        return result + ", " + prefix
    return result

def _helsing_a_v(parts):
    prefix = ''.join([aChar == "'t" and aChar or aChar+"." for aChar in getInitials(parts.prefix)])
    initials = parts.initials.split('.', 1)[0] + '.'
    result = "%s, %s" % (_capitalize(parts.lastname), _capitalize(initials))
    if prefix:
        return result + " " + prefix
    return result

def _helsing_ab_van(parts):
    if parts.initials:
        result = "%s, %s" % (_capitalize(parts.lastname), _capitalize(parts.initials))
    else:
        result = _capitalize(parts.lastname)
    if parts.prefix:
        return result + " " + parts.prefix
    return result
...
def firstLetter(unparsedName):
    parts = _breakUp(unparsedName)
    if parts:
        yield str(unicode(parts.lastname)[:1].upper())
...
familyname = lastname
def givenname(unparsedName):
    parts = _breakUp(unparsedName)
    if parts:
        yield _capitalize(parts.givenname)
